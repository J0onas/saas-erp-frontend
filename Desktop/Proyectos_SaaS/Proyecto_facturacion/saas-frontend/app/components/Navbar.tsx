...
const NAV_ITEMS = [
  ...
  { href: '/sucursales',    label: 'Sucursales',   icon: Icons.branches,  roles: ['GERENTE', 'SUPERADMIN'] },
...
export default function Navbar({ cajaInfo, onCerrarTurno }: {
  ...
  return (
    <aside
      ...
      className="fixed left-0 top-0 h-screen z-40 flex flex-col bg-[#0f172a] border-r border-white/5 transition-all duration-300 ease-in-out overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 h-16 border-b border-white/5 flex-shrink-0">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          {!collapsed && (
            <span className="text-white font-bold text-sm tracking-wide whitespace-nowrap">SaaS POS</span>
          )}
        </div>
        <button onClick={toggleCollapsed}
          className="text-slate-500 hover:text-slate-300 p-1 rounded-lg hover:bg-white/5 transition flex-shrink-0">
          <div className="w-4 h-4">{collapsed ? Icons.chevronRight : Icons.chevronLeft}</div>
        </button>
      </div>
